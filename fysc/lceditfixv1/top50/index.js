            ...
            if (json.users) {
                if (json.users.length > 0) {
                    for (let i = 0; i < json.users.length; i++) {
                        ...
                        if (hasID == false) {
                            fetch('https://nia-statistics.com/api/get?platform=youtube&type=channel&id=' + json.users[i].id + '')
                                .then(response => response.json())
                                .then(json2 => {
                                    let count = 0;
                                    let name = json2.snippet.title;
                                    let image = json2.snippet.thumbnails[2].url;
